...
void processor::dispatch() {
   ...
   for (i = 0; i < dispatch_width; i++) {
      ...
      PAY.buf[index].lane_id = steer(PAY.buf[index].fu);
      ...
      if(PAY.buf[index].C_int && PAY.buf[index].C_valid)
      {     
         int_dest_flag = true;
         fp_dest_flag = false;
      }
      else if(PAY.buf[index].C_valid)
      {
         int_dest_flag = false;
         fp_dest_flag = true;
      }
...
unsigned int processor::steer(fu_type fu) {
