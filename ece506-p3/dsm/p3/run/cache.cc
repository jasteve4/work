...
dirEntry *Directory::findEntry(ulong tag) {
    uint i =0;
    bool found = false;
...
dirEntry *Directory::findUncached(ulong tag) {
    uint i = 0;
    for (i = 0; i < size; i++) {
        if (entry[i].state == U)
            break;
    ...
    if (i== size) {
        cout << "ERROR: no uncached (empty) directory entry found" << endl;
        exit(0);
    }
    entry[i].tag = tag;
...
void Cache::printStats(int processorNumber) { 
	cout << " Simulation results (Cache " << processorNumber << ")" << endl;
        ...
        printf("number of reads, number of read misses, number of writes, write misses, total miss rate, number of writebacks, number of Invalidations, number of cache-to-cache transfers\n");         
	cout << (int) getReads();
	cout << ", ";
	cout << (int) getRM();
	cout << ", ";
