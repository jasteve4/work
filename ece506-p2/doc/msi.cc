...
bus_op_t Msi::PrRd(ulong addr, int processorNumber) 
{
...
bus_op_t Msi::PrWr(ulong addr, int processorNumber) 
{
    cache_state state;
    currentCycle++;
    writes++;
	...
	if (state==M) 
	    ...
	    line->set_state(M);
            busRdXs++;
	    return busRdX; // Write hit in Sc or SM --> BusUpd
	}
	else return none; // Write hit in state M, no bus action
    }
}

cacheLine * Msi::allocateLine(ulong addr) { 
    ulong tag;
    cache_state state;
